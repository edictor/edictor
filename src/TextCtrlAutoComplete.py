...
class FakePopupWindow(wx.MiniFrame):
	...
	def Position(self, position, size):
		#print("pos2=%s size=%s" % (position, size))
	...
	def OnKeyDown(self, evt):
		...
		"""
		#print "testepp2"
		print "On KeyDown", evt.GetKeyCode(), wx.WXK_RETURN
		evt.Skip()
...
class TextCtrlAutoComplete (wx.TextCtrl, listmix.ColumnSorterMixin ):
    ...
    def __init__ ( self, parent, ID=-1, tmp='', colNames=None, choices = ['empty'],
        ...
        #widgets
        self._mac = bool(wx.Platform == '__WXMAC__')
        if self._mac:
    ...
    def onListKeyDown ( self, event ) :
        """ Allows user to select an item by pressing ENTER
        """
        KC = event.GetKeyCode()
        print 'epa!', KC
...
#            self._multiChoices.sort(key=lambda x: locale.strxfrm(x[0]).lower() )
    ...
    def SetChoices(self, choices):
        ...
        flags = wx.LC_REPORT | wx.LC_SINGLE_SEL | wx.LC_NO_HEADER # | wx.LC_SORT_ASCENDING
        self.dropdownlistbox.SetWindowStyleFlag(flags)

        if not isinstance(choices, list):
            self._choices = [ x for x in choices]

        #prevent errors on "old" systems (COMENTADO POR PPFF, em 29/05/09)
#        if sys.version.startswith("2.3"):
